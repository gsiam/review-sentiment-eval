...
**The unresolvable residual is the *blind spot* form:** shared training could cause both model instances to miss the same error class entirely, with no low-scored cases as evidence. A third-family judge (e.g., OpenAI, Gemini) is the only way to probe this.

**Mitigation — covers both**: a cross-family judge is the non-negotiable baseline. It partially addresses leniency by breaking stylistic coupling between summariser and judge, and partially addresses blind spots by breaking shared-training correlation. Neither problem is fully eliminated — a cross-family judge can still have its own leniency or blind spots — but it removes the specific same-family correlated failure mode. Not currently in this dataset; see [§8 rec. 6](#8-recommendations).
